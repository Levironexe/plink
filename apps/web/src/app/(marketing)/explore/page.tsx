        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {creators.map((c, i) => (
            <Reveal key={c.username} delay={Math.min(i, 8) * 50}>
              <Link
                href={`/${c.username}`}
                className="group flex h-full flex-col rounded-xl border border-line bg-surface p-5 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-lift"
              >
