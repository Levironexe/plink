...
import { formatMoney, initialsOf, isReservedUsername } from "@plink/core/utils";
      ...
      <div className="mx-auto max-w-3xl px-5 py-12 sm:py-20">
        ...
        <header className="mt-8 flex flex-wrap items-center gap-5">
          ...
          <div className="min-w-0">
            ...
            <h1 className="mt-1 text-[clamp(1.9rem,4.6vw,2.8rem)] leading-tight font-semibold text-ink">
              {user.displayName}
            </h1>
        ...
        {kit.about && (
          <section className="mt-10 rounded-xl border border-line bg-surface p-6 shadow-soft">
            <h2 className="font-mono text-[12px] leading-4 text-ink-muted">About</h2>
        ...
        {filledStats.length > 0 && (
          <section className="mt-6">
            <h2 className="font-mono text-[12px] leading-4 text-ink-muted">Audience</h2>
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {filledStats.map(([label, value]) => (
                <div key={label} className="rounded-xl border border-line bg-surface p-4 shadow-soft">
                  <p className="text-[12.5px] font-semibold text-ink-muted">{label}</p>
                  <p className="mt-1 text-[22px] leading-tight font-semibold text-ink">{value}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        {kit.rateCard.length > 0 && (
          <section className="mt-6">
            <h2 className="font-mono text-[12px] leading-4 text-ink-muted">Rate card</h2>
            <div className="mt-3 divide-y divide-line overflow-hidden rounded-xl border border-line bg-surface shadow-soft">
              {kit.rateCard
              ...
              {total > 0 && (
                <div className="flex items-center justify-between gap-4 bg-canvas px-5 py-4">
                  ...
                  <span className="text-[16px] font-semibold text-ink">{formatMoney(total)}</span>
                </div>
              )}
            </div>
          </section>
        ...
        {kit.brands.length > 0 && (
          <section className="mt-6">
            <h2 className="font-mono text-[12px] leading-4 text-ink-muted">Previously worked with</h2>
        ...
        {user.socials.length > 0 && (
          <section className="mt-6">
            <h2 className="font-mono text-[12px] leading-4 text-ink-muted">Channels</h2>
            <ul className="mt-3 flex flex-wrap gap-2">
              {user.socials.map((s) => {
        ...
        <section className="mt-10 rounded-xl bg-ink p-7 text-center">
          ...
          <a
            ...
            className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-brand-500 px-5 py-3 text-[15px] font-bold text-ink transition hover:brightness-95"
          >
