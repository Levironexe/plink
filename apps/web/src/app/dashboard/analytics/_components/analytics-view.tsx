...
import { RANGES } from "@plink/core/analytics-shared";
import type { AnalyticsSummary } from "@/lib/analytics";
import { PageHeader, EmptyState } from "../../_components/page-header";
import { cn, compactNumber, formatMoney, hostOf } from "@plink/core/utils";
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6 lg:py-10">
      ...
      {!data.hasData ? (
        ...
        <>
          <section className="mt-6 rounded-xl border border-line bg-surface p-5 shadow-soft sm:p-6">
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-line bg-surface p-5 shadow-soft">
          <p className="inline-flex items-center gap-2 font-mono text-[12px] leading-4 text-ink-muted">
            <Wallet className="size-3.5" aria-hidden /> Revenue in range
          </p>
          <p className="mt-2 text-[32px] leading-none font-semibold text-ink">
            {formatMoney(data.totals.revenueCents)}
          </p>
          <p className="mt-1.5 text-[13.5px] text-ink-muted">{data.totals.orders} orders and tips</p>
        </div>
        <div className="rounded-xl border border-line bg-surface p-5 shadow-soft">
          <p className="font-mono text-[12px] leading-4 text-ink-muted">Reporting window</p>
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-line bg-surface p-5 shadow-soft">
        ...
        {delta !== undefined && (
          <span
            className={cn(
              ...
              up ? "bg-[#e9fbef] text-brand-500" : "bg-[#fff1ee] text-[#c2410c]",
            )}
          >
      ...
      <p className="mt-4 text-[30px] leading-none font-semibold text-ink">{value}</p>
