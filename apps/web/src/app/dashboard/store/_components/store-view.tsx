...
import { FileUpload } from "@plink/ui/file-upload";
...
import { cn, formatMoney, relativeTime } from "@plink/core/utils";
    ...
    <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6 lg:py-10">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.5fr_1fr]">
            ...
            <div className="mt-4 flex flex-col gap-3">
              {products.map((p) => (
                <article
                  key={p.id}
                  className={cn(
                    "flex items-center gap-4 rounded-xl border border-line bg-surface p-4 shadow-soft",
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-0.5">
                    <button
                      ...
                      className="rounded-lg px-2 py-1.5 text-[12.5px] font-bold text-ink-muted transition hover:bg-canvas-deep hover:text-ink"
                    >
                    ...
                    <button
                      ...
                      className="rounded-lg p-2 text-ink-muted transition hover:bg-canvas-deep hover:text-ink"
                    >
        ...
        <section>
          ...
          <div className="mt-4 divide-y divide-line overflow-hidden rounded-xl border border-line bg-surface shadow-soft">
            {orders.length === 0 && (
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <FileUpload
            ...
            hint="Buyers get a private, expiring download link after checkout."
          />
          <Toggle
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-line bg-surface p-5 shadow-soft">
      <p className="font-mono text-[12px] leading-4 text-ink-muted">{label}</p>
      <p className="mt-2 text-[30px] leading-none font-semibold text-ink">{value}</p>
    </div>
  );
}
