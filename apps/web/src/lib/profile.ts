...
import { liveBlocks } from "@plink/core/scheduling";
...
export async function loadPublicProfile(username: string): Promise<PublicProfile | null> {
  ...
  const user = await prisma.user.findUnique({
    ...
    include: {
      ...
      // startsAt/endsAt come along so `liveBlocks` can apply the schedule window.
  ...
  const theme: ThemeShape = user.theme
    ? {
        ...
        bgEffect: user.theme.bgEffect,
        textEffect: user.theme.textEffect,
        entranceEffect: user.theme.entranceEffect,
  ...
  return {
    ...
    theme,
    blocks: liveBlocks(user.blocks).map((b) => ({
      id: b.id,
      type: b.type,
      title: b.title,
      subtitle: b.subtitle,
