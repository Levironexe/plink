...
- [ ] T1 — Spike: the image API in the installed `ai` package, the gateway
      model id, and the degrade-never-crash policy
      (`docs/spikes/2026-09-03-ai-sdk-image-generation.md`).
- [ ] T2 — Spec / plan / tasks under `docs/specs/asset-generator/`, including
      the sanctioned `GeneratedAsset` shape change.
- [ ] T3 — `packages/ai/src/assets.ts`: `ASSET_KINDS`, `ASSET_PROMPT_MAX`,
...
- [ ] T4 — `apps/web/tests/unit/ai-assets.test.ts`: prompt clamping and
...
- [ ] T5 — `apps/web/src/app/api/assets/route.ts`: POST (guards → generate →
      upload → `Asset` row → audit + event) and GET (list per site).
- [ ] T6 — `/studio/[siteId]/assets`: server page + `_components/asset-studio.tsx`
      (kind picker, prompt form with pending state, gallery grid with kind
      badge / prompt / date / copy-URL, "not configured" state).
- [ ] T7 — Full gate: `pnpm --filter @plink/web typecheck && lint && test` and
