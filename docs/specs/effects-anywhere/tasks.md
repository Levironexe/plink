...
- [ ] T1. Spike: record CSS-only text-effect decisions in
      `docs/spikes/2026-09-03-css-only-text-effects.md`.
- [ ] T2. Registry: add `target` to `EffectDefinition` (type-only `EffectTarget` import
...
- [ ] T3. CSS: 7 text effects (`pl-fx-text-*`) — gradient, shimmer, typewriter
      (clip-path steps), blur-reveal, wave, glitch (steps + clip), highlight.
- [ ] T4. CSS: 7 background effects (`pl-fx-bg-*`) on the `::before` layer — aurora,
      beams, dot-grid, grid, mesh-drift, noise, gradient-flow.
- [ ] T5. CSS: 7 entrance effects (`pl-fx-enter-*`) animating only from
      `[data-entered]`, including `enter-stagger` child delays; extend the
      reduced-motion block for the new selectors.
- [ ] T6. Tests: extend `apps/web/tests/unit/effects.test.ts` — target partition,
...
- [ ] T7. Components: `EntranceGroup` in
      `apps/web/src/components/effects/entrance-group.tsx`.
- [ ] T8. Components: `EffectPicker` in
      `apps/web/src/components/effects/effect-picker.tsx` + `index.ts` barrel.
- [ ] T9. Verify: `pnpm --filter @plink/web typecheck && lint && test` green; confirm
