...
- [ ] T1 — Contract artifacts: `spec.md`, `plan.md`, `tasks.md` under
      `docs/specs/proposal-edited-metric/` (constitution VII.1).
- [ ] T2 — Spike: why the signal fires at publish time rather than through an
...
- [ ] T3 — Store: private `recordAiProposalOutcome` helper in
      ...
      Never throws.
- [ ] T4 — Wire it into `publishSite` after the snapshot transaction commits;
      publish result unchanged.
- [ ] T5 — Unit tests `apps/web/tests/unit/proposal-metric.test.ts` (mocked
...
- [ ] T6 — One-line update to `docs/specs/website-generator/spec.md`
      *Future work*, pointing `ai_proposal_edited` at what shipped.
- [ ] T7 — Full gate: `pnpm --filter @plink/web typecheck && lint && test`.
