...
- [ ] T1 — Spike: diff semantics decision record
      (`docs/spikes/2026-09-03-diff-semantics-versioning.md`).
- [ ] T2 — Pure core: `packages/core/src/site-versioning.ts`
      (`nextVersionNumber`, `diffDocuments`, `stableStringify` internal).
      `pnpm --filter @plink/core typecheck` green.
- [ ] T3 — Core unit tests in `apps/web/tests/unit/versioning.test.ts`:
...
- [ ] T4 — Store: `apps/web/src/lib/site-store.ts` — `requireSiteAccess`,
      `getSiteForUser`, `saveDraft`, `publishSite`, `rollbackSite`,
      `listVersions`, `writeAudit`, `logEvent`.
- [ ] T5 — Store unit tests (same file, mocked `@plink/db` + `@/lib/auth`):
...
- [ ] T6 — API routes: `versions` (GET), `publish` (POST), `rollback`
