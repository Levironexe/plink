  ...
  return (
    <div className="w-full">
      ...
      <input
        ...
        className={cn("field", error && "border-danger focus:border-danger focus:shadow-[0_0_0_1px_var(--color-danger)]", className)}
        {...props}
      />
      {error ? (
        <p id={`${id}-err`} className="mt-1.5 text-[13px] text-danger">{error}</p>
      ) : hint ? (
  ...
  return (
    <div className="w-full">
      ...
      <textarea
        id={id}
        aria-invalid={error ? true : undefined}
        className={cn("field resize-y min-h-[92px]", error && "border-danger", className)}
        {...props}
      />
      {error ? (
    ...
    <label className={cn("flex items-start gap-3", disabled ? "opacity-50" : "cursor-pointer")}>
      <button
        ...
        className={cn(
          ...
          checked ? "bg-ink" : "bg-line-strong/50",
        )}
      >
        <span
          className={cn(
      ...
      {(label || description) && (
        <span className="min-w-0">
          {label && <span className="block text-[14px] font-medium tracking-[-0.02em] text-ink">{label}</span>}
