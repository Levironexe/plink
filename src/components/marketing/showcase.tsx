  ...
  return (
    <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
      <Reveal className={cn(flip && "lg:order-2")}>
        ...
        <h3 className="mt-5 text-[clamp(1.8rem,3.6vw,2.6rem)] leading-[1.06] font-extrabold text-ink">{title}</h3>
...
function ThemeVisual() {
  return (
    <div className="rounded-[28px] border border-line bg-surface p-6 shadow-soft">
      <p className="text-[13px] font-bold tracking-wider text-ink-muted uppercase">Themes</p>
  ...
  return (
    <div className="rounded-[28px] border border-line bg-surface p-6 shadow-soft">
      <div className="flex items-baseline justify-between">
        <p className="text-[13px] font-bold tracking-wider text-ink-muted uppercase">Your store</p>
      ...
      <div className="mt-4 space-y-3">
        {PRODUCTS.map((p) => (
          <div key={p.name} className="flex items-center gap-3.5 rounded-2xl border border-line bg-canvas p-3 transition hover:border-line-strong">
            ...
            <span className="rounded-full bg-ink px-3 py-1.5 text-[13px] font-bold text-white">
              {formatMoney(p.price)}
            </span>
          </div>
        ))}
      ...
      <div className="mt-4 rounded-2xl bg-brand-50 p-4 text-center">
        <p className="text-[13px] font-semibold text-brand-700">0% platform fee on Pro</p>
      </div>
    </div>
  );
}
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="rounded-2xl bg-canvas p-3.5">
            ...
            <p className="mt-1 text-[21px] leading-none font-extrabold text-ink">{s.value}</p>
            <p className="mt-1.5 text-[12px] font-bold text-[#15803d]">{s.delta}</p>
          </div>
        ))}
      </div>
      <div className="mt-6 flex h-40 items-end gap-1.5">
        {BARS.map((h, i) => (
          <div key={i} className="flex flex-1 flex-col items-center gap-1.5">
            <div
              className="w-full rounded-t-md bg-gradient-to-t from-brand-300 to-brand-500 transition-all duration-500"
              style={{ height: `${h}%` }}
            />
        ...
        {[
          ...
          <div key={l.name}>
            <div className="flex justify-between text-[13px]">
              ...
              <span className="font-bold text-ink-muted">{l.pct}%</span>
            </div>
  ...
  return (
    <div className="rounded-[28px] border border-line bg-surface p-6 shadow-soft">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-[18px] font-bold text-ink">Maya Osei</p>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <div key={s.k} className="rounded-xl bg-canvas p-3">
            ...
            <p className="mt-0.5 text-[17px] font-extrabold text-ink">{s.v}</p>
          </div>
        ))}
      </div>
      <p className="mt-5 text-[13px] font-bold tracking-wider text-ink-muted uppercase">Rate card</p>
      <div className="mt-2.5 divide-y divide-line rounded-2xl border border-line">
        {[
          ...
          <div key={label as string} className="flex items-center justify-between px-4 py-3">
            ...
            <span className="text-[14.5px] font-bold text-ink">{formatMoney(Number(price) * 100)}</span>
          </div>
        ))}
      </div>
    </div>
